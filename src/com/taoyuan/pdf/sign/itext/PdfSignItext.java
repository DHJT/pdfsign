package com.taoyuan.pdf.sign.itext;
...
public class PdfSignItext {
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * try { //读取keystore ，获得私钥和证书链 KeyStore ks = KeyStore.getInstance("PKCS12");
		 * ks.load(new FileInputStream(new File(KEYSTORE)), PASSWORD); String alias =
		 * (String)ks.aliases().nextElement(); PrivateKey pk = (PrivateKey)
		 * ks.getKey(alias, PASSWORD); Certificate[] chain =
		 * ks.getCertificateChain(alias); //new一个上边自定义的方法对象，调用签名方法 PdfSignItext app =
		 * new PdfSignItext(); app.sign(new FileInputStream(SRC), new
		 * FileOutputStream(DEST), chain, pk, DigestAlgorithms.SHA1, null,
		 * CryptoStandard.CMS, "数据不可更改", "桃源乡",chapterPath); } catch (Exception e) {
	...
	public static void sign(InputStream src // 需要签章的pdf文件路径
			...
			char[] password, String reason // 签名的原因，显示在pdf签名属性中，随便填
